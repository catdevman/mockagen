...
Every push to `master` also publishes the head medians to the `gh-pages` branch
...
- **`master` regressions** open (or comment on) an issue labelled
...
1. Push this to `master`. The first run creates the `gh-pages` branch.
...
make bench-compare BASE=master    # same interleaved A/B as CI, then the report
